    ...
    <div className="space-y-6">
      ...
      <div className="bg-linear-to-r from-gray-900 to-gray-800 rounded-lg p-5 border border-gray-700">
        <div className="flex flex-col sm:flex-row items-start gap-4">
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="font-semibold text-white truncate">{certificate.student?.name}</div>
            <div className="text-xs text-gray-400 truncate">{certificate.student?.email}</div>
          ...
          <div className="sm:ml-auto ml-0 flex flex-col sm:items-end items-start gap-3 w-full sm:w-auto">
            ...
            {certificate.status === 'issued' && certificate.fileUrl ? (
              <div className="flex flex-col sm:flex-row gap-2 w-full sm:w-auto">
                <button onClick={() => handleDownload(certificate._id, certificate.enrollment?.course?.title)} disabled={downloading} className="w-full sm:w-auto px-3 py-1 rounded bg-indigo-600 text-white text-sm flex items-center justify-center gap-2"><CheckCircle className="w-4 h-4" /> {downloading ? 'Downloading...' : 'Download'}</button>
                <button onClick={handlePreview} disabled={downloading} className="w-full sm:w-auto px-3 py-1 rounded bg-gray-800 text-white text-sm">Preview</button>
                <button onClick={() => navigate('/admin/certificates')} className="w-full sm:w-auto px-3 py-1 rounded bg-gray-700 text-white text-sm">Back to list</button>
              </div>
            ) : null}
          </div>
        </div>
        ...
        <div className="bg-gray-800 rounded p-4 border border-gray-700">
          ...
          <form onSubmit={handleUpload} className="mt-3 space-y-3">
            <div className="flex flex-col sm:flex-row items-stretch gap-2">
              ...
              <input id="certificate-file" aria-label="Certificate PDF file" accept="application/pdf" type="file" onChange={(e) => setFile(e.target.files ? e.target.files[0] : null)} className="text-sm w-full sm:w-auto" />
              <button type="submit" disabled={uploading || !file} className="w-full sm:w-auto px-3 py-1 rounded bg-indigo-600 text-white text-sm">{uploading ? 'Issuing...' : 'Upload & Issue'}</button>
            </div>
            <div className="text-xs text-gray-400">OR</div>
            <div className="flex flex-col sm:flex-row gap-2">
              <button type="button" onClick={handleGenerate} disabled={uploading} className="w-full sm:w-auto px-3 py-1 rounded bg-yellow-600 text-black text-sm">{uploading ? 'Generating...' : 'Generate & Issue'}</button>
            </div>
            <div>
      ...
      {certificate.status !== 'rejected' && (
        <div className="bg-gray-800 rounded p-4 border border-gray-700">
          ...
          <div className="mt-3 space-y-2">
            ...
            <div className="flex flex-col sm:flex-row gap-2">
              <button onClick={handleReject} className="w-full sm:w-auto px-3 py-1 rounded bg-red-600 text-white text-sm">Reject Request</button>
              <button onClick={() => navigate('/admin/certificates')} className="w-full sm:w-auto px-3 py-1 rounded bg-gray-700 text-white text-sm">Back to list</button>
            </div>
          </div>
        </div>
      ...
      {previewOpen && previewUrl && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-60">
          <div className="bg-gray-900 rounded w-11/12 md:w-3/4 max-h-[90vh] p-4 overflow-hidden flex flex-col">
