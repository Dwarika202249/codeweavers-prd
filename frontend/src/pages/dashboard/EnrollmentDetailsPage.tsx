...
import { enrollmentAPI, downloadAPI, submissionsAPI } from '../../lib/api';
...
export default function EnrollmentDetailsPage() {
    ...
    // Assignments state
    const [assignments, setAssignments] = useState<Array<{ assignment: any; submission?: any }>>([]);
    const [assignLoading, setAssignLoading] = useState(false);

    // Per-assignment submission inputs (GitHub URL)
    const [assignmentInputs, setAssignmentInputs] = useState<Record<string, { link?: string; notes?: string; submitting?: boolean }>>({});

    const setAssignmentInput = (assignmentId: string, update: Partial<{ link?: string; notes?: string; submitting?: boolean }>) => {
        setAssignmentInputs((prev) => ({
            ...prev,
            [assignmentId]: { ...(prev[assignmentId] || {}), ...update },
        }));
    };

    const submitAssignment = async (assignmentId: string) => {
        const input = assignmentInputs[assignmentId] || {};
        const link = (input.link ?? '').trim();
        if (!link) return showError('Please provide the GitHub URL');
        setAssignmentInput(assignmentId, { submitting: true });
        try {
            await submissionsAPI.submit(assignmentId, { enrollmentId: enrollment._id, link, notes: input.notes || '' });
            showSuccess('Submission saved');
            // clear inputs for this assignment
            setAssignmentInput(assignmentId, { link: '', notes: '', submitting: false });
            fetchAssignments();
        } catch (err: any) {
            showError(err?.message || 'Submit failed');
            console.error('Submit error', err);
            setAssignmentInput(assignmentId, { submitting: false });
        }
    };
    ...
    const fetchAssignments = async () => {
        ...
        try {
            const res = await enrollmentAPI.getAssignments(id!);
    ...
    const fetch = async () => {
        ...
        setCertLoading(true);
        try {
            const [enRes, certRes] = await Promise.all([enrollmentAPI.getById(id), enrollmentAPI.getCertificate(id)]);
    ...
    useEffect(() => { fetch(); fetchAssignments(); }, [id]);
    ...
    return (
        <div className="space-y-6">
            ...
            <div className="flex flex-col sm:flex-row items-start sm:items-start gap-4">
                ...
                {/* Assignments moved to the curriculum's right column (bottom-right) */}
            </div>
                ...
                <div className="bg-gray-800 rounded p-4">
                    ...
                    <div className="mt-3 space-y-3">
                        {assignLoading ? (
                            ...
                            <div className="text-gray-400">No assignments for this course</div>
                        ) : (
                            assignments.map(({ assignment: a, submission }) => (
                                <div key={a._id} className="rounded border border-gray-700 bg-gray-900 p-3">
                                    <div className="flex items-start justify-between">
                                        <div className="min-w-0">
                                            ...
                                            <div className="text-xs text-gray-400">Due: {a.dueDate ? new Date(a.dueDate).toLocaleDateString() : '—'}</div>
                                        </div>
                                        <div className="text-xs text-gray-400">{submission ? (submission.status === 'graded' ? `Graded: ${submission.score}` : submission.status) : 'Not submitted'}</div>
                                    </div>

                                    <div className="mt-3">
                                        <div className="space-y-2 text-xs text-gray-300">
                                            <div>Link: {submission?.link ? (<a className="text-indigo-400 hover:underline" href={submission.link} target="_blank" rel="noreferrer">{submission.link}</a>) : '—'}</div>
                                            <div>Notes: {submission?.notes || '—'}</div>

                                            {(a.allowResubmissions || !submission) ? (
                                                <form onSubmit={(e) => { e.preventDefault(); submitAssignment(a._id); }} className="mt-2 sm:flex sm:items-center sm:gap-2">
                                                    <input aria-label={`GitHub URL for ${a.title}`} value={assignmentInputs[a._id]?.link ?? submission?.link ?? ''} onChange={(e) => setAssignmentInput(a._id, { link: e.target.value })} placeholder="GitHub URL" className="w-full sm:flex-1 rounded bg-gray-800 px-2 py-1 text-white text-sm" />
                                                    <input aria-label="Notes (optional)" value={assignmentInputs[a._id]?.notes ?? submission?.notes ?? ''} onChange={(e) => setAssignmentInput(a._id, { notes: e.target.value })} placeholder="Notes (optional)" className="mt-2 sm:mt-0 rounded bg-gray-800 px-2 py-1 text-white text-sm w-full sm:w-56" />
                                                    <div className="mt-2 sm:mt-0">
                                                        <button type="submit" disabled={assignmentInputs[a._id]?.submitting} className="px-3 py-1 rounded bg-indigo-600 text-white text-sm">{assignmentInputs[a._id]?.submitting ? 'Submitting...' : submission ? 'Resubmit' : 'Submit'}</button>
                                                    </div>
                                                </form>
                                            ) : null}
                                        </div>
